 ...
 * 
 * Additional Features:
 * - FE1) Added Ability to Select an exisiting Group Vs Creating a new one.
 * - FE2) Added Ability to select an existing Event Occurrence vs Creating a new one.
 * - FE3) Added Ability to pass a reservation id via query string to add a reservation linkage as well
 * - FE4) Added "New" Button at the end that allows users to step through the processes again.  This will keep a lot of the fields the same they were set to before.
 * - FE5) Added the ability to limit which locations are selectable under the Group Selection Selection screen by the Location's type. i.e. Campus.
 * - FE6) Addeed the ability to make Registrations Optional
 * - UI1) Added the option to use the Registration Template Picker ( This option doesn't allow the ability to filter the avalible templates via the block settings ).
 * - UI2) Added the option to add Help Text to each step.
 */

namespace RockWeb.Plugins.com_razayya.Blocks.Event
{
    /// <summary>
    /// A wizard to simplify creation of Event Registrations.
    /// </summary>
    [DisplayName( "Event Registration Wizard" )]
    [Category( "Razayya > Event" )]
    ...
    /* FE1.Start */
    [BooleanField(
        "Allow Selecting an Existing Group",
        Key = CustomAttributeKey.AllowSelectingAnExistingGroup,
        Description = "Should the user be able to select an existing Group",
        DefaultValue = "False",
    ...
    [BooleanField(
        ...
        Key = CustomAttributeKey.RequireRegistration,
        Description = "If Registrations are Required, a Template and Registration must be selected.",
        DefaultValue = "True",
        Category = "Additional Features" )]
    /* FE1.End */
    ...
    /* FE2.Start */
    [BooleanField(
        "Allow Selecting an Existing Event Occurrence",
        Key = CustomAttributeKey.AllowSelectingAnExistingEventOccurrence,
        Description = "Should the user be able to select an existing Event Occurrence",
        DefaultValue = "False",
        Category = "Additional Features" )]
    /* FE2.End */
    ...
    [BooleanField(
        "Show New Button",
        Key = CustomAttributeKey.ShowNewButton,
        Description = "Should users be shown the 'New' Button at after completeing the wizard",
        DefaultValue = "False",
        Category = "Additional Features" )]
    /* FE4.End */
    ...
    [DefinedValueField(
        "Limit Group Location by Location Type"
        , Key = CustomAttributeKey.LimitGroupLocation
        , AllowAddingNewValues = false
        , AllowMultiple = true
        , Category = "Additional Features"
        , DefaultValue = ""
        , DefinedTypeGuid = Rock.SystemGuid.DefinedType.LOCATION_TYPE
        , Description = "Limit the options for selection for Group Locations by selecting what types of locations can be selected."
    ...
    [BooleanField(
        "Use Registration Template Picker",
        Key = CustomAttributeKey.UseRegistrationTemplatePicker,
        Description = "Should the Registartion Template Picker Control be used vs the Drop Down.  Selection this option will negate the ability to filter which templates are shown.",
        DefaultValue = "False",
        Category = "Additional Features" )]
    ...
    [CodeEditorField(
        "Registration Template Help Text",
        "Help text to display when on the Registration Template Screen",
        CodeEditorMode.Html,
        CodeEditorTheme.Rock,
        DefaultValue = "",
        Category = "Help Text",
        Key = CustomAttributeKey.RegistrationTemplateHelpText,
        Order = 1,
        IsRequired = false
        )]
    ...
    [CodeEditorField(
        ...
        "Help text to display when on the Registration Screen",
        CodeEditorMode.Html,
        CodeEditorTheme.Rock,
        DefaultValue = "",
        Category = "Help Text",
        Key = CustomAttributeKey.RegistrationHelpText,
        Order = 2,
        IsRequired = false
        )]
    ...
    [CodeEditorField(
        "Group Help Text",
        "Help text to display when on the Group Screen",
        CodeEditorMode.Html,
        CodeEditorTheme.Rock,
        DefaultValue = "",
        Category = "Help Text",
        Key = CustomAttributeKey.GroupHelpText,
        Order = 3,
        IsRequired = false
        )]
    ...
    [CodeEditorField(
        "Event Help Text",
        "Help text to display when on the Event Screen",
        CodeEditorMode.Html,
        CodeEditorTheme.Rock,
        DefaultValue = "",
        Category = "Help Text",
        Key = CustomAttributeKey.EventHelpText,
        Order = 4,
        IsRequired = false
        )]

    [CodeEditorField(
        "Event Occurrence Help Text",
        "Help text to display when on the Event Occurrence Screen",
        CodeEditorMode.Html,
        CodeEditorTheme.Rock,
        DefaultValue = "",
        Category = "Help Text",
        Key = CustomAttributeKey.EventOccurrenceHelpText,
        Order = 5,
        IsRequired = false
        )]
    ...
    public partial class EventRegistrationWizard : RockBlock
    {
        ...
        protected static class CustomAttributeKey
        {
            public const string AllowSelectingAnExistingGroup = "AllowSelectingAnExistingGroup";
            public const string AllowSelectingAnExistingEventOccurrence = "AllowSelectingAnExistingEventOccurrence";
            public const string ShowNewButton = "ShowNewButton";
            public const string LimitGroupLocation = "LimitGroupLocation";
            public const string UseRegistrationTemplatePicker = "UseRegistrationTemplatePicker";
            public const string RegistrationTemplateHelpText = "RegistrationTemplateHelpText";
            public const string RegistrationHelpText = "RegistrationHelpText";
            public const string GroupHelpText = "GroupHelpText";
            public const string EventHelpText = "EventHelpText";
            public const string EventOccurrenceHelpText = "EventOccurrenceHelpText";
            public const string RequireRegistration = "RequireRegistration";
        }
        #endregion
        ...
        {
            /* FE4.Start */
            if ( GetAttributeValue( CustomAttributeKey.ShowNewButton ).AsBoolean() )
            {
                lbNext_Summary.Text = "Submit";
            }
            else
            {
                lbNext_Summary.Text = "Finish";
            }
            /* FE4.End */
            ...
            /* FE1.Start */
            if ( !GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean()
                || ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean()
                    && !toggleExistingGroup.Checked ) )
            {
                /* FE1.END */
            ...
            else if ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean() && toggleExistingGroup.Checked && gpGroup.SelectedValueAsInt() != null )
            {
                var groupService = new GroupService( rockContext );
                var group = groupService.Get( gpGroup.SelectedValueAsInt().Value );
                if ( group != null )
                {
                    string groupDescription = string.Format( "{0} will be used.", group.Name );
            ...
            if ( ( tglEventSelection.Checked ) || ( eipSelectedEvent.SelectedValueAsId() != null ) )
            {
                ...
                {
                    /* FE2.Start */
                    if ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingEventOccurrence ).AsBoolean() && toggleExistingOccurrence.Checked && ddlEventOccurrence.SelectedValueAsInt().HasValue )
                    {
                        eventDescription = "The event occurrence \"" + ddlEventOccurrence.SelectedItem.Text + "\" will be used. ";
                    }
                    else
                    {
                ...
                if ( reservationId.HasValue )
                {
                    ...
                    var reservation = reservationService.Get( reservationId.Value );
                    if ( reservation != null )
                    {
                        string reservationDescription = "A Room Reservation link to " + reservation.Name + " will be created.";
                        var reservationLiteral = new Literal() { Text = string.Format( itemTemplate, "Reservation", reservationDescription ) };
                        phChanges.Controls.Add( reservationLiteral );
                    }
                }
        ...
        private CommitResult CommitChanges( RockContext rockContext )
        {
            ...
            if ( GetSelectedTemplate() != null )
            {
                ...
                if ( !GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean()
                   || ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean()
                       && !toggleExistingGroup.Checked ) ) /*FE1.ENd */
                {
                    if ( !string.IsNullOrWhiteSpace( tbGroupName.Text ) )
                    {
                        var groupService = new GroupService( rockContext );
                ...
                else if ( gpGroup.SelectedValueAsInt() != null && GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean() && toggleExistingGroup.Checked )
                {
                    var groupService = new GroupService( rockContext );
                    var group = groupService.Get( gpGroup.SelectedValueAsInt().Value );
                    ...
                    if ( GroupTypeIsCheckinGroup( group.GroupTypeId, rockContext ) )
                    {
                        if ( lpGroupLocation.Location != null )
                        {
                            ...
                            var groupLocation = group.GroupLocations.Where( x => x.LocationId == lpGroupLocation.Location.Id ).FirstOrDefault();
                            ...
                            if ( selectedSchedules.Any() )
                            {
                                ...
                                foreach ( int selectedScheduleId in selectedSchedules )
                                {
                                    var groupLocationSchedule = groupLocation.Schedules.Where( x => x.Id == selectedScheduleId ).FirstOrDefault();
                                    if ( groupLocationSchedule == null )
                                    {
                                        groupLocationSchedule = scheduleService.Get( selectedScheduleId );
                                        groupLocation.Schedules.Add( groupLocationSchedule );
                                    }
                                }
                            }
            ...
            if ( GetAttributeValue( AttributeKey.EnableCalendarEvents ).AsBoolean() )
            {
                ...
                if ( eventItem != null )
                {
                    ...
                    if ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingEventOccurrence ).AsBoolean() && toggleExistingOccurrence.Checked )
                    {

                        var eventItemOccurrenceId = ddlEventOccurrence.SelectedValueAsInt();

                        if ( eventItemOccurrenceId.HasValue )
                        {
                            linkage.EventItemOccurrenceId = eventItemOccurrenceId.Value;
                            result.EventOccurrenceId = eventItemOccurrenceId.Value.ToString();
                        }
            ...
            var reservationId = PageParameter( "ReservationId" ).AsIntegerOrNull();
            ReservationLinkageService reservationLinkageService;
            ...
            if ( reservationId.HasValue )
            {
                ...
                if ( reservation != null )
                {
                    reservationLinkageService = new ReservationLinkageService( rockContext );
                    ReservationLinkage reservationLinkage = new ReservationLinkage();
                    reservationLinkage.Reservation = reservation;
                    reservationLinkage.ReservationId = reservation.Id;
                    reservationLinkage.EventItemOccurrenceId = linkage.EventItemOccurrenceId;
        ...
        private void SetResultLinks( CommitResult result )
        {
            ...
            liReservationLink.Visible = reservationId.HasValue;
            if ( reservationId.HasValue )
            {
                hlReservation.NavigateUrl = "/ReservationDetail?ReservationId=" + reservationId.Value.ToString();
            }
        ...
        private bool ValidateGroupSelection( RockContext rockContext )
        {
            ...
            if ( groupId != null )
            {
                ...
                if ( group != null )
                {
                    bool isAuthorized = group.IsAuthorized( Authorization.ADMINISTRATE, CurrentPerson ) ||
                    ...
                    else
                    {
                        ...
                        var groupType = groupTypeService.Get( group.GroupTypeId );

                        bool isGroupType = groupType.Id == templateGroupType.Id;
                        if ( !isGroupType )
                        {
                            nbNotPermitted.Text = string.Format( "{0} is configured for Groups with the {1} type. {2} is has the {3} type.", registrationTemplate.Name, templateGroupType.Name, group.Name, groupType.Name );
        ...
        private void BindEventOccurrences()
        {
            var eventId = eipSelectedEvent.SelectedValueAsInt();

            ddlEventOccurrence.Items.Clear();

            if ( ( !GetAttributeValue( AttributeKey.AllowCreatingNewCalendarEvents ).AsBoolean() || !tglEventSelection.Checked ) && eventId.HasValue )
            {
                toggleExistingOccurrence.Enabled = true;
                toggleExistingOccurrence.Visible = true;

                RockContext rockContext = new RockContext();
                EventItemService eventItemService = new EventItemService( rockContext );

                var eventItem = eventItemService.Get( eventId.Value );

                if ( eventItem != null )
                {
                    foreach ( var item in eventItem.EventItemOccurrences.Where( x => x.NextStartDateTime.HasValue ).OrderByDescending( x => x.NextStartDateTime ) )
                    {
                        ddlEventOccurrence.Items.Add( new ListItem()
                        {
                            Value = item.Id.ToString()
                            ,
                            Text = item.ToString() + ( item.NextStartDateTime.HasValue ? " " + item.NextStartDateTime.Value.ToString( "M/d/yyyy" ) : "" )
                        } );
                    }
                }
            ...
            {
                toggleExistingOccurrence.Enabled = false;
                toggleExistingOccurrence.Visible = false;
                pnlNewOccurrence.Visible = true;
                pnlExistingOccurrence.Visible = false;
                toggleExistingOccurrence.Checked = false;
            }
        }
        ...
        private bool ValidateGroupLocationSelection()
        {
            var LocationTypeGuids = GetAttributeValue( CustomAttributeKey.LimitGroupLocation ).Split( ',' ).AsGuidList();

            if ( lpGroupLocation.Location != null )
            {

                Guid? SelectedLocationTypeGuid = null;

                if ( lpGroupLocation.Location.LocationTypeValueId.HasValue )
                {
                    SelectedLocationTypeGuid = DefinedValueCache.Get( lpGroupLocation.Location.LocationTypeValueId.Value ).Guid;
                }

                if ( LocationTypeGuids.Count > 0 )
                {
                    if ( !SelectedLocationTypeGuid.HasValue || !LocationTypeGuids.Contains( SelectedLocationTypeGuid.Value ) )
                    {
                        nbGroupLocationTypeNotPermitted.Text = string.Format( "{0} is a not a valid location type Location Type.", lpGroupLocation.Location.Name );
                        nbGroupLocationTypeNotPermitted.Visible = true;
                        return false;
                    }
                    else
                    {
                        nbGroupLocationTypeNotPermitted.Visible = false;
                    }
                }
            }
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                {
                    /* UI1.Start */
                    Init_SetRegistrationTemplateControl();
                    /* UI1.End */
                    Init_SetContact();
        ...
        private void Init_SetRegistrationTemplateControl()
        {
            bool useTemplatePicker = GetAttributeValue( CustomAttributeKey.UseRegistrationTemplatePicker ).AsBoolean();
            /* FE6.Start */
            bool isRequired = GetAttributeValue( CustomAttributeKey.RequireRegistration ).AsBoolean();
            /* FE6.End */
            if ( useTemplatePicker )
            {
                ddlTemplate.Visible = false;
                ddlTemplate.Required = false;
                rtpTemplate.Visible = true;
                rtpTemplate.Required = isRequired;
            }
            else
            {
                ddlTemplate.Visible = true;
                ddlTemplate.Required = isRequired;
                rtpTemplate.Visible = false;
                rtpTemplate.Required = false;
            }
        }
        ...
        /* UI2.Start */
        private void Init_HelpText()
        {
            var mergeObjects = LavaHelper.GetCommonMergeFields( this.RockPage, this.CurrentPerson );

            string registrationTemplateHelpText = GetAttributeValue( CustomAttributeKey.RegistrationTemplateHelpText );
            phRegistrationTemplateHelpText.Controls.Add( new LiteralControl( registrationTemplateHelpText.ResolveMergeFields( mergeObjects ) ) );

            string registrationHelpText = GetAttributeValue( CustomAttributeKey.RegistrationHelpText );
            phRegistrationHelpText.Controls.Add( new LiteralControl( registrationHelpText.ResolveMergeFields( mergeObjects ) ) );

            string groupHelpText = GetAttributeValue( CustomAttributeKey.GroupHelpText );
            phGroupHelpText.Controls.Add( new LiteralControl( groupHelpText.ResolveMergeFields( mergeObjects ) ) );

            string eventHelpText = GetAttributeValue( CustomAttributeKey.EventHelpText );
            phEventHelpText.Controls.Add( new LiteralControl( eventHelpText.ResolveMergeFields( mergeObjects ) ) );

            string eventOccurrenceHelpText = GetAttributeValue( CustomAttributeKey.EventOccurrenceHelpText );
            phEventOccurrenceHelpText.Controls.Add( new LiteralControl( eventOccurrenceHelpText.ResolveMergeFields( mergeObjects ) ) );
        }
        /* US2.End */
        ...
        private void ShowInputPanel( ActiveWizardStep step )
        {
            ...
            switch ( step )
            {
                ...
                case ActiveWizardStep.Group:
                    ...
                    pnlNewGroup.Visible = !( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean() && toggleExistingGroup.Checked );
                    pnlExistingGroup.Visible = GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean() && toggleExistingGroup.Checked;
                    break;
                case ActiveWizardStep.Event:
        ...
        protected void lbNext_Group_Click( object sender, EventArgs e )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                /* FE1.Start */
                if ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingGroup ).AsBoolean() && toggleExistingGroup.Checked )
                {
                    groupSelectionIsValid = groupSelectionIsValid && ValidateGroupSelection( rockContext );
                }
                /* FE1.End */

                /* FE5.Start */

                groupSelectionIsValid = groupSelectionIsValid && ValidateGroupLocationSelection();
        ...
        protected void lbPrev_Event_Click( object sender, EventArgs e )
        {
            ...
            else if( GetSelectedTemplate() != null )
            {
                SetActiveWizardStep( ActiveWizardStep.Registration );
            }
            else
        ...
        protected void lbNext_Event_Click( object sender, EventArgs e )
        {
            ...
            if ( ( tglEventSelection.Checked ) || ( eipSelectedEvent.SelectedValueAsId() != null ) )
            {
                SetActiveWizardStep( ActiveWizardStep.EventOccurrence );
                /* FE2.Start */
                if ( GetAttributeValue( CustomAttributeKey.AllowSelectingAnExistingEventOccurrence ).AsBoolean() )
                {
                    BindEventOccurrences();
                }
                /* FE2.END */
            }
            else
        ...
        protected void lpGroupLocation_SelectLocation( object sender, EventArgs e )
        {
            ValidateGroupLocationSelection();
        }
        ...
        protected void gpGroup_SelectItem( object sender, EventArgs e )
        {
            using ( var rockContext = new RockContext() )
            {
                ValidateGroupSelection( rockContext );
            }
        }

        protected void toggleExistingGroup_CheckedChanged( object sender, EventArgs e )
        {
            pnlExistingGroup.Visible = toggleExistingGroup.Checked;
            pnlNewGroup.Visible = !toggleExistingGroup.Checked;
        }
        /* FE1.End */

        /* FE2.Start */
        protected void ddlEventOccurrence_SelectItem( object sender, EventArgs e )
        {
        }

        protected void toggleExistingOccurrence_CheckedChanged( object sender, EventArgs e )
        {
            pnlExistingOccurrence.Visible = toggleExistingOccurrence.Checked;
            pnlNewOccurrence.Visible = !toggleExistingOccurrence.Checked;
        }
        /* FE2.END */

        /* UI1.Start */

        protected void rtpTemplate_SelectItem( object sender, EventArgs e )
        {
            ddlTemplate.SetValue( rtpTemplate.SelectedValueAsInt() );
            var selectedTemplateId = rtpTemplate.SelectedValueAsInt();
            if ( selectedTemplateId == null )
            {
                pnlCosts.Visible = true;
                pnlDefaultPayment.Visible = true;
                /* FE6.Start */
                SaveSelectedTemplate( null );
                /* FE6.End */
                return;
            }
