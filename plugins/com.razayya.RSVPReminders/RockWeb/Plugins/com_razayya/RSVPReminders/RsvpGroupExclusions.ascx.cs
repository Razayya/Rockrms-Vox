...
namespace RockWeb.Plugins.com_razayya.RSVPReminders
{
    ...
    [Description( "Lets group leaders skip meeting dates so RSVP emails aren't sent for those occurrences. On the Group Toolbox it renders as a link to the dedicated Meeting Exclusions page; on that page it renders the full manager. Only shows for AutoRSVP-enabled groups the current person can manage." )]
    ...
    public partial class RsvpGroupExclusions : RockBlock
    {
        ...
        /// <summary>
        /// Resolves the route group and gates everything: GroupId param present, group
        /// has a schedule, current person can manage the group, and the group is
        /// AutoRSVP-enabled (RSVP group-type tree + SendsRsvpEmails attribute) — the
        /// same identification the AutoRSVP job uses. Returns null (nothing renders)
        /// otherwise.
        /// </summary>
