...
namespace com.razayya.GroupFinder.Controllers
{
    /// <summary>
    /// The controller class for the GroupTools
    /// </summary>
    [RoutePrefix( "api/com_razayya/GroupFinder" )]

    public class RazayyaGroupFinderController : Rock.Rest.ApiControllerBase
    {
            ...
            bool useDefinedValues = true)
        {
            ...
            var qryOptions = new GroupQueryOptions
            {
                ...
                GrabDefinedValues = useDefinedValues,
        ...
        private static void BuildGroupSchedule( Group group, GroupInformation groupInfo )
        {
            if ( group.Schedule != null )
            {
                ...
                {
                    CalendarEvent calendarEvent = schedule.GetICalEvent();
            ...
            GroupQueryOptions qryOptions )
        {
            ...
            {
                qry = FilterOnAttributes( rockContext, definedValueService, qry, qryOptions.GrabDefinedValues, qryOptions.CategoryKey, categoryIdList );
            }
            ...
            if ( qryOptions.OptionalFilterAttributeKey.IsNotNullOrWhiteSpace() && qryOptions.OptionalFilterIds.IsNotNullOrWhiteSpace() )
            {
                ...
                {
                    qry = FilterOnAttributes( rockContext, definedValueService, qry, qryOptions.GrabDefinedValues, qryOptions.OptionalFilterAttributeKey, optionalFilterIdList );
                }
            }
            ...
            {
                var secondaryCategoryIdList = qryOptions.SecondaryCategoryFilterIds.SplitDelimitedValues().AsIntegerList();
                if ( secondaryCategoryIdList.Any() )
                {
                    qry = FilterOnAttributes( rockContext, definedValueService, qry, qryOptions.GrabDefinedValues, qryOptions.SecondaryCategoryAttributeKey, secondaryCategoryIdList );
                }
            }
            ...
            if( qryOptions.Zipcode.IsNotNullOrWhiteSpace() )
            {
                var locationService = new LocationService( rockContext );
                try
                {
                    var coordinates = locationService.GetMapCoordinateFromPostalCode( qryOptions.Zipcode );
                    if ( coordinates.Latitude.HasValue && coordinates.Longitude.HasValue )
                    {
                        var location = new Location();
                        location.SetLocationPointFromLatLong( coordinates.Latitude.Value, coordinates.Longitude.Value );

                        double metersFrom = qryOptions.milesFrom * Location.MetersPerMile;

                        qry = qry.Where( g => g.GroupLocations.Any( l => l.Location.GeoPoint.Distance( location.GeoPoint ) <= metersFrom ) );
        ...
        private static IQueryable<Group> FilterOnAttributes( RockContext rockContext, DefinedValueService definedValueService, IQueryable<Group> qry, bool grabDefinedValues, string attributeKey, List<int> valueIdList )
        {
            var valueList = valueIdList.Select( a => a.ToString() );
            if ( grabDefinedValues )
            {
                valueList = definedValueService.GetByIds( valueIdList ).Select( c => c.Guid.ToString().ToUpper() ).ToList();
            }

            return qry.WhereAttributeValue( rockContext, av => av.Attribute.Key == attributeKey && valueList.Any( c => av.Value.ToUpper().Contains(c) ) );
        }
    ...
    public class GroupQueryOptions
    {
        ...
        public bool GrabDefinedValues { get; set; } = true;
    }
