...
namespace com.razayya.JourneyTrack.Field.Types
{
    /// <summary>
    /// Field type that stores a JourneyProgram reference as a Guid string.
    /// Renders as a dropdown of active Journey Programs.
    /// </summary>
    public class JourneyProgramFieldType : FieldType, IEntityFieldType
    {
        #region Formatting
        ...
        public override string GetTextValue( string privateValue, Dictionary<string, string> privateConfigurationValues )
        {
            ...
            if ( guid.HasValue )
            {
                ...
                {
                    var group = new JourneyProgramService( rockContext ).Get( guid.Value );
                    if ( group != null )
                    {
                        return group.Name;
                    }
        ...
        public void SetEditValueFromEntityId( Control control, Dictionary<string, ConfigurationValue> configurationValues, int? id )
        {
            ...
            if ( id.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    var group = new JourneyProgramService( rockContext ).Get( id.Value );
                    if ( group != null )
                    {
        ...
        public Rock.Data.IEntity GetEntity( string value, RockContext rockContext )
        {
            ...
            if ( guid.HasValue )
            {
                ...
                return new JourneyProgramService( rockContext ).Get( guid.Value );
            }
